...
/* ───────────────────────── Helpers ───────────────────────── */
function isWhorl(code) { return W_CODES.includes(code); }
function isLoop(code) { return L_CODES.includes(code); }
function isRadial(code) { return R_CODES.includes(code); }
function isArch(code) { return X_CODES.includes(code); }
...
export function interpretATD(angle) {
    if (angle < 35)
        return { speed: 'Very Fast Learner', level: 5 };
    if (angle < 40)
        return { speed: 'Fast Learner', level: 4 };
    if (angle < 45)
        return { speed: 'Average Learner', level: 3 };
    if (angle < 50)
        return { speed: 'Moderate Learner', level: 2 };
    if (angle < 55)
        return { speed: 'Slow Learner', level: 1 };
    return { speed: 'Needs Support', level: 0 };
}
function getWeights(patternCode) {
...
function getDominantDISC(discProfile) {
...
function interpretQuotient(val, type) {
    if (type === 'iq') {
        if (val >= 130)
    ...
    if (type === 'aq') {
        if (val >= 8)
            return 'Climber (High Resilience)';
        if (val >= 6)
            return 'Resilient';
        if (val >= 4)
            return 'Camper (Moderate)';
        if (val >= 2)
            return 'Developing';
...
function interpretRC(rc) {
    if (rc === 0)
        return { level: 'Arch (Zero)', description: 'This brain region functions through compensation from other areas. Not a weakness — other pathways handle this function.' };
    if (rc < 8)
        return { level: 'Lower', description: 'Less natural inclination in this area, but it can be developed with practice and patience.' };
    if (rc < 15)
        return { level: 'Average', description: 'Balanced development in this brain region. A solid foundation that responds well to effort.' };
...
/* ───────────────────────── Main Calculate ───────────────────────── */
export function calculateDmit(input) {
    const { patterns, ridgeCounts, atdLeft, atdRight } = input;
    const codes = patterns;
    // Manifested raw calculations
    const manifestedRaw = Object.fromEntries(FINGER_INTELLIGENCE.map(k => [k, 0]));
    const fingerManifested = codes.map((code, i) => {
    ...
    const intelligenceData = Object.keys(intelligenceNames).map(k => ({
        key: k,
    ...
    // Career list
    const careerList = generateCareerList(rbDist, manifestedRaw);
    ...
    const fingerRCAnalysis = fingerBrainMap.map((f, i) => ({
        ...f,
        pattern: codes[i],
        ridgeCount: ridgeCounts[i],
        rcInterpretation: interpretRC(ridgeCounts[i]),
    }));
    const fingerPairComparison = [];
    for (let i = 0; i < 5; i++) {
        ...
        if (leftRC === 0 && rightRC === 0)
            ...
            balance = 'Left brain stronger';
        else
            balance = 'Right brain stronger';
    ...
    const inv = (v) => Math.max(0, 2 * meanIntel - v);
    ...
    Object.keys(bigFive).forEach(k => {
        ...
        bigFive[k].percent = p;
        bigFive[k].interpretation = interpretBigFive(k, p);
    });
    // Strengths profile
...
function generateCareerList(rbDist, manifestedRaw) {
...
function interpretBigFive(key, p) {
    const ranges = {
        O: [
...
/* ───────────────────────── Strengths Generator ───────────────────────── */
function generateTopStrengths(topIntellKeys) {
    const strengthsMap = {
...
function getDecisionMaking(disc) {
    const styles = {
...
function getConflictStyle(disc) {
    const styles = {
...
function getFeedbackPreference(disc) {
    const prefs = {
...
function getMotivationDrivers(disc) {
    const drivers = {
...
function getRiskTolerance(disc) {
    const tolerances = {
...
function getTeamRole(disc, _strengthsDomain) {
    const roles = {
...
function getTransferableSkills(topIntellKeys) {
    const skillMap = {
    ...
    const skills = [];
    topIntellKeys.forEach(k => {
        skillMap[k]?.forEach(s => { if (!skills.includes(s))
            skills.push(s); });
    });
    return skills.slice(0, 10);
}
function getBridgeCareers(topIntellKeys, _hollandTop2, _streamTop) {
    const bridgeMap = {
    ...
    const careers = [];
    topIntellKeys.forEach(k => {
        bridgeMap[k]?.forEach(c => { if (!careers.includes(c))
            careers.push(c); });
    });
    return careers.slice(0, 8);
}
function getCognitiveFitnessRecommendations(topIntellKeys) {
    const recMap = {
...
function getStressSignature(disc) {
    const sigs = {
...
function getRecoveryActivities(topIntellKeys, disc) {
    const discRec = {
...
function getPurposeAlignment(topIntellKeys) {
    const purposeMap = {
    ...
    return topIntellKeys.slice(0, 3).map(k => purposeMap[k]).filter(Boolean);
}
/* ───────────────────────── SWOT, Acquiring, Extra, Lobes, Sub-intelligences ───────────────────────── */
function generateSWOT(intelligenceData, dominantDISC, brainSide) {
...
function generateAcquiringMethods(discProfile, learningStyle, sensory) {
    ...
    const methods = [];
    if (topLearning === 'Kinesthetic')
        methods.push({ name: 'Experiential Learning', desc: 'You learn best by doing. Hands-on labs, building projects, and physical practice are your fastest paths to mastery.', color: '#ef4444' });
    if (topLearning === 'Auditory')
        methods.push({ name: 'Auditory Acquisition', desc: 'Lectures, discussions, and verbal explanations work best for you. Record and replay key concepts.', color: '#f59e0b' });
    if (topLearning === 'Visual')
        methods.push({ name: 'Visual Learning', desc: 'Diagrams, charts, videos, and written notes are your best tools. Use color-coding and mind maps.', color: '#3b82f6' });
    if (topDisc === 'D')
        methods.push({ name: 'Action-Oriented', desc: 'You acquire knowledge best when there is a clear goal and immediate application. Case studies and competitive exercises help.', color: '#10b981' });
    if (topDisc === 'I')
        methods.push({ name: 'Social Learning', desc: 'Group study, teaching others, and interactive workshops accelerate your learning. You need discussion and energy.', color: '#8b5cf6' });
    if (topDisc === 'S')
        methods.push({ name: 'Reflective Practice', desc: 'You need time to absorb information. Written summaries, repetition, and step-by-step guides work best.', color: '#06b6d4' });
    if (topDisc === 'C')
        methods.push({ name: 'Structured Study', desc: 'Detailed documentation, research papers, and systematic courses suit you. You need evidence and logical flow.', color: '#f43f5e' });
    if (topSense === 'Touch')
        methods.push({ name: 'Tactile Engagement', desc: 'Manipulating objects, writing by hand, and physical models enhance your retention.', color: '#14b8a6' });
    if (topSense === 'See')
        methods.push({ name: 'Observational Learning', desc: 'Watching demonstrations and studying visual examples gives you strong recall.', color: '#6366f1' });
    return { primary: methods[0], secondary: methods[1] || methods[0], all: methods };
}
function generateExtraCurricular(topIntellKeys, discP) {
    const activityMap = {
    ...
    const activities = [];
    topIntellKeys.forEach(k => {
        activityMap[k]?.forEach(a => { if (!activities.includes(a))
            activities.push(a); });
    });
    ...
    discActivities[discP]?.forEach(a => { if (!activities.includes(a))
        activities.push(a); });
    return activities.slice(0, 10);
}
function generateBrainLobes(fingerRCAnalysis) {
...
function generateSubIntelligences(rbDist) {
    const subs = [
...
//# sourceMappingURL=dmitEngine.js.map