# DMIT Pro - cPanel Deployment Build Script (PowerShell)
# Run this in PowerShell from the project root

Write-Host "🚀 Building DMIT Pro for cPanel Deployment..." -ForegroundColor Green
...
# Copy API (built dist + package.json + drizzle config)
Write-Host "📋 Preparing API package..." -ForegroundColor Cyan
New-Item -ItemType Directory -Path "$DeployDir\api" | Out-Null
Copy-Item -Recurse "apps\api\dist" "$DeployDir\api\"
Copy-Item "apps\api\package.json" "$DeployDir\api\"
Copy-Item "apps\api\drizzle.config.ts" "$DeployDir\api\"
New-Item -ItemType Directory -Path "$DeployDir\api\src\db" -Force | Out-Null
Copy-Item -Recurse "apps\api\src\db\migrations" "$DeployDir\api\src\db\"

# Copy shared package dist
Write-Host "📋 Preparing shared package..." -ForegroundColor Cyan
New-Item -ItemType Directory -Path "$DeployDir\packages\shared" | Out-Null
Copy-Item -Recurse "packages\shared\dist" "$DeployDir\packages\shared\"
Copy-Item "packages\shared\package.json" "$DeployDir\packages\shared\"

# Copy web frontend (static files)
Write-Host "📋 Preparing web frontend..." -ForegroundColor Cyan
New-Item -ItemType Directory -Path "$DeployDir\web" | Out-Null
Copy-Item -Path "apps\web\dist\*" -Destination "$DeployDir\web\" -Recurse
...
'@
$EnvContent | Out-File -FilePath "$DeployDir\api\.env.example" -Encoding UTF8
...
'@
$Instructions | Out-File -FilePath "$DeployDir\DEPLOY_INSTRUCTIONS.md" -Encoding UTF8

Write-Host ""
Write-Host "✅ cPanel deployment package created in: $DeployDir/" -ForegroundColor Green
Write-Host ""
...
Write-Host "3. Follow DEPLOY_INSTRUCTIONS.md" -ForegroundColor White
Write-Host ""
